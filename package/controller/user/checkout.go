...
func CheckOut(c *gin.Context) {
	...
	if couponCode != "" {
		...
		for _, val := range cartItems {
			...
			if err := initializer.DB.Where("user_id =? AND product_id=?", userId, val.ProductId).Delete(&models.Cart{}); err.Error != nil {
...
func OrderView(c *gin.Context) {
	...
	userId := c.GetUint("userid")
	initializer.DB.Where("user_id=?", userId).Joins("Product").Find(&orders)
...
func OrderDetails(c *gin.Context) {
	...
	userId := c.GetUint("userid")
	initializer.DB.Preload("Product").Where("id=?", userId).First(&order)
	c.JSON(200, gin.H{
		"Product":         order.Product.Name,
		"Amount":          order.OrderAmount,
